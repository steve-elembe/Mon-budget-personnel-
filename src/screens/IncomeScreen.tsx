...
export const IncomeScreen: React.FC<IncomeScreenProps> = ({
  ...
  const defaultCategoryName = incomeCategories.length > 0 ? incomeCategories[0].name : 'Salaire';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTouched({});
    setErrors({});
    setSuccessMsg('Revenu enregistré avec succès dans la base de données Room');
    setTimeout(() => setSuccessMsg(''), 3500);
  };

  const selectedCategoryObj = incomeCategories.find(c => c.name === category);

  return (
    <div className="space-y-6 p-4 sm:p-6">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 rounded-3xl bg-linear-to-r from-emerald-600 via-emerald-700 to-teal-800 text-white shadow-lg">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/15 text-xs font-semibold backdrop-blur-xs mb-2">
            ...
            <span>Flux de Revenus • Jetpack Compose MVVM</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black">Gestion des Revenus</h2>
          <p className="text-xs sm:text-sm text-emerald-100/90 mt-0.5">
            Enregistrement automatique avec validation et catégories dynamiques
          </p>
        </div>
        <div className="sm:text-right bg-white/10 backdrop-blur-xs p-3.5 sm:px-5 rounded-2xl border border-white/10">
          <p className="text-xs text-emerald-200 font-medium">Total cumulé des revenus</p>
          <p className="text-2xl sm:text-3xl font-black tracking-tight">{formatFCFA(totalIncome)}</p>
        </div>
      </div>
      ...
      {/* Main Grid: Add Form + History List */}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                Description / Origine <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={description}
                ...
                placeholder="Ex: Salaire de base, Ventes en boutique..."
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                Catégorie de revenu <span className="text-red-500">*</span>
              </label>
                ...
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  {selectedCategoryObj ? (
                    <div
                      ...
                      style={{ backgroundColor: selectedCategoryObj.color || '#10b981' }}
                    >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-9 pr-8 py-2 text-xs sm:text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/80 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  placeholder="Ajoutez une note, numéro de chèque, transaction ID..."
                  className="w-full pl-9 pr-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/80 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full py-2.5 px-4 rounded-xl font-bold text-xs sm:text-sm text-white bg-emerald-600 hover:bg-emerald-700 active:scale-98 shadow-md shadow-emerald-600/25 transition-all flex items-center justify-center gap-2"
            >
              <Plus className="w-4 h-4" />
              <span>Enregistrer le revenu</span>
            </button>
          </form>
        </div>

        {/* Income History (7 cols on lg) */}
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">
                {incomeTransactions.length} transaction(s) enregistrée(s)
              </p>
            </div>
            <span className="text-xs font-bold px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300">
          ...
          {incomeTransactions.length === 0 ? (
            <div className="py-12 text-center text-slate-400">
              <p className="text-sm font-medium">Aucun revenu enregistré pour le moment</p>
              <p className="text-xs mt-1">Utilisez le formulaire ci-contre pour ajouter votre premier revenu.</p>
            </div>
          ) : (
            <div className="space-y-2.5 overflow-y-auto max-h-[500px] pr-1">
              {incomeTransactions.map(tx => {
                const catObj = incomeCategories.find(c => c.name === tx.category);
                return (
                  <div
                    key={tx.id}
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      >
                        <CategoryIcon name={catObj?.icon || 'Briefcase'} className="w-5 h-5" />
                      </div>
